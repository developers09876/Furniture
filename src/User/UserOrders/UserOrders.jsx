...
const UserOrders = () => {
  ...
  const [isOrderModel, setOrderModel] = useState(false);
  const [userOrder, setUserOrder] = useState([]);
  const [userMap, setUserMap] = useState({}); // To map user IDs to usernames
  ...
  const columns = [
    ...
    {
      title: "Username", // Added Username column
      render: (text, record) => (
        <span>{userMap[record.user_id] || "Unknown User"}</span> // Assuming user_id is available in the order record
      ),
    },
    {
      title: "Delivery Company",
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        setData(response.data);
        setDataFilter(response.data);
        // Extract user IDs from orders to fetch user data
        const userIds = response.data.map(order => order.user_id);
        fetchUsers(userIds); // Fetch usernames based on user IDs
        setLoading(false);
      })
      .catch((error) => {
        console.error("Error fetching the order data", error);
  ...
  const fetchUsers = (userIds) => {
    // Make an API call to get user details for the specified user IDs
    axios
      .get(`${import.meta.env.VITE_MY_API}users/getUsers`, { params: { ids: userIds } }) // Assuming this endpoint exists
      .then((response) => {
        // Map user IDs to usernames
        const users = response.data.reduce((acc, user) => {
          acc[user.id] = user.username; // Assuming the response has user.id and user.username
          return acc;
        }, {});
        setUserMap(users);
      })
