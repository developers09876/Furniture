...
const Users = () => {
  ...
  const handleDelete = (record) => {
    confirm({
      ...
      cancelText: "No",
      onOk() {
        setData((prevData) => prevData.filter((item) => item.key !== record.key)); // Remove the user from data
  ...
  return (
    <StyledUsers>
      ...
      <Modal centered open={Adduser} onCancel={() => setAdduser(false)} footer={null}>
        <AddUser setAdduser={setAdduser} />
      </Modal>
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleEditSubmit} initialValues={selectedUser}>
          <Form.Item
            name="name"
            label="Name"
            rules={[{ required: true, message: "Please enter the name" }]}
